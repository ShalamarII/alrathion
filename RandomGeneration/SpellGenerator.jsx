...
function SpellQuerySetting({ title, icon, children}) {
	return <>
		<div style="border: 1px solid var(--background-modifier-border); border-radius: 8px; padding: 0.5em 0.5em; min-width: 150px; background-color: var(--background-secondary);">
			<div style="font-weight: 600; font-size: 0.9em; margin-bottom: 0.5em; display: flex; justify-content: center; gap: 0.5em;">
				<dc.Icon icon={icon} />
				<span>{title}</span>
			</div>
...
const allRarities = [ "Common", "Uncommon", "Rare", "Very Rare", "Virtually Unknown", "Unique", "Lost" ];
...
function SpellQuery({ type = allClasses, rarity = allRarities, level, restricted = true, paging = 20, showFilters: showFilters = true }) {
	...
	const filteredPages = dc.useMemo(() => (
		allPages.filter(page => {
			...
			if (!filterRestricted && page.$tags.some(t => t.includes("#trait/restricted"))) return false;
	...
	return <>
		<div style="display: flex; gap: 0.75em;">
			...
			{showFilters && <button class="primary" onclick={() => setFiltersShown(!filtersShown)}>⚙</button>}
			{showFilters && <button onclick={() => {
				setFilterClass(type);
				setFilterLevel(level);
				setFilterSearch('');
				setFilterSchools(allSchools);
				setFilterSpheres(allSpheres);
				setFilterComponents(allComponents);
				setFilterRarity(rarity);
				setFilterRestricted(restricted);
			}}>Clear All</button>}
		</div>
		{filtersShown && <div>
			<SpellQuerySettings>
				...
				<SpellQuerySetting title="School" icon="lucide-folder">
				...
				<SpellQuerySetting title="Sphere" icon="lucide-circle-dot">
				...
				<SpellQuerySetting title="Components" icon="lucide-puzzle">
				...
				<SpellQuerySetting title="Rarity" icon="lucide-star">
